...
const quizData = [
    ...
    },
    {
        ...
        options: [
            { text: "जीवित जीवाणु", correct: false },
...
let state = {
    ...
    currentIndex: 0,
    answers: {}, // Key: original id, Value: { selectedIdx: num, correct: bool, timeTaken: num }
    startTime: 0,
    timerInterval: null,
    totalSeconds: 0,
    ...
    audioSpeed: 1.0,
    autoRead: false,
    audioPositions: {}, // Map of audio file URL -> last playback position
...
// Elements (Bug Fixed: Mapped resume buttons correctly here)
const elements = {
    ...
    resumeYesBtn: document.getElementById('resume-yes-btn'), // Map Added
    resumeNoBtn: document.getElementById('resume-no-btn'),   // Map Added
...
// ============================================================================
// AUDIO MANAGER (Plays Single Question MP3 File with Resuming Support)
// ============================================================================
class AudioManager {
    constructor() {
        ...
        if (this.currentAudio) {
            try {
                ...
                if (this.currentAudio.src) {
                    state.audioPositions[this.currentAudio.src] = this.currentAudio.currentTime;
                }
            } catch (err) {}
            this.currentAudio = null;
        }
    ...
    playFile(url, onEndedCallback) {
        ...
        // Resume playback if we have a saved timestamp
        if (state.audioPositions[url]) {
            audio.currentTime = state.audioPositions[url];
        }

        audio.addEventListener('ended', () => {
            state.audioPositions[url] = 0; // Clear finished
    ...
    fallbackTTS(url, callback) {
        ...
        // TTS fallback: Speaks question and then options sequentially
...
function startSequence(fromBeginning = false) {
    ...
    if (fromBeginning) {
        state.audioPositions[currentQ.questionAudio] = 0; // Reset timeline
    }
...
function renderCurrentQuestion() {
    ...
    // Use actual original ID for badge rendering (e.g., "प्रश्न 8")
    ...
    if (state.autoRead) {
        setTimeout(() => {
            startSequence(true); // Always start from beginning on new question
        }, 300);
    }
}
...
function renderQuestionGrid() {
    ...
    state.questions.forEach((q, idx) => {
        ...
        btn.textContent = q.id; // Shows exact original ID on the grid (e.g. 1, 2, 3, 4, 5, 6, 8, 9, 10, 12...)
